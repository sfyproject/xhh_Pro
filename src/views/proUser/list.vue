...
<script>
import { listProUser, deleteProUser } from '@/api/proUser'
...
export default {
  ...
  methods: {
    getList() {
      ...
      listProUser(this.listQuery).then(response => {
    ...
    handleCreate() {
      this.$router.push({ path: '/proUser/create' })
    },
    handleUpdate(row) {
      this.$router.push({ path: '/proUser/edit', query: { id: row.id }})
    },
    showDetail(detail) {
    ...
    handleDelete(row) {
      deleteProUser(row).then(response => {
        this.$notify.success({
          title: '成功',
          message: '删除成功'
        })
